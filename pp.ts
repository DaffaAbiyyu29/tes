      ...
      for (const incident of incidents) {
        ...
        if (incident.UnitSerialNumber) {
          ...
          const totalActualHours =
            (dataUnit?.vwProcess
              .flatMap((proc) =>
                proc.vwProcessAssign.flatMap((assign) =>
                  assign.vwProcessActivity.map((activity) =>
                    Number(activity.ProcessActivityActualHours || 0)
                  )
                )
              )
              .reduce((sum, minutes) => sum + minutes, 0) || 0) / 3600; // convert ke jam

          const mhUsage = (totalActualHours / standardMH) * 100;
          const productionProgress = 0

          mhDiscrepancy.push({
            no: mhDiscrepancy.length + 1,
            unitSN: incident.UnitSerialNumber,
            mhUsage: `${mhUsage.toFixed(2)}%`,
